...
namespace Exchange {
  ...
  struct MEOrdersAtPrice {
    ...
    MEOrder *first_me_order_ = nullptr; // first order in the priority list; other orders follow
  ...
  // Max price levels = ME_MAX_PRICE_LEVELS, so we'll 
  //    add elements in (price)mod(ME_MAX_PRICE_LEVELS)
