...
namespace Exchange {
  MarketDataPublisher::MarketDataPublisher(MEMarketUpdateLFQueue *market_updates, const std::string &iface,
      ...
      : outgoing_md_updates_(market_updates), snapshot_md_updates_(ME_MAX_MARKET_UPDATES),
        run_(false), logger_("/usr/outputs/exchange_market_data_publisher.log"), incremental_socket_(logger_) {
  ...
  auto MarketDataPublisher::run() noexcept -> void {
    ...
    while (run_) {
      for (auto market_update = outgoing_md_updates_->getNextToRead();
        ...
        logger_.log("%:% %() % Sending seq:% %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), next_inc_seq_num_ /*just dummy variable for counting*/,
                    market_update->toString().c_str());

        incremental_socket_.send(&next_inc_seq_num_, sizeof(next_inc_seq_num_)); // send sequence number to socket
        incremental_socket_.send(market_update, sizeof(MEMarketUpdate)); // send update message to socket
        outgoing_md_updates_->updateReadIndex(); // marked as read

        // For every read message, get new info (seq. and message) from the snapshop (replenish?)
